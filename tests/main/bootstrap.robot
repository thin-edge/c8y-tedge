...
Bootstrap main device to the cloud
    ${result}=    Process.Run Process    docker compose exec c8y zsh -c "c8y tedge bootstrap root@tedge '${DEVICE_ID}'"
    ${result}=    Process.Run Process    docker    compose    exec    c8y    zsh    -c    c8y tedge bootstrap root@tedge '${DEVICE_ID}'    cwd=${CURDIR}/../..
